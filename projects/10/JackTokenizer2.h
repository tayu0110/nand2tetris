...
#ifndef JACK_JACKTOKENIZER2_H
#define JACK_JACKTOKENIZER2_H
...
class JackTokenizer {
  ...
  int tokenType(std::string);
  std::string stringVal(std::string);
public:
  JackTokenizer() : isComment(false), isStrConst(false), jackcode(""), tokens(0), pos(0), len(0) {}
